...
Главным результатом успешного выполнения make является создание исполняемого файла. Его имя зависит от проекта.

Для известных проектов: имя часто соответствует названию программы (например, vim, git, python).

В общем случае: имя исполняемого файла по умолчанию часто бывает a.out, но в большинстве проектов это меняется на что-то осмысленное.
